...
<body>
    ...
    <div class="thread container my-5" style="word-wrap : break-word;">
        <h5 class="my-3"> Posted by :
            <a href="./users.php?uid=<?php echo $row['owner_id_'];?>">
    ...
    <form id="postCommentContainer" action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post" class="mb-3">
        <?php
        // WE ALLOW TO POST COMMENT ONLY IF USER IS LOGGED IN , SO CHECK IT 
            ...
            // ASK TO LOGIN FOR POSTING AND RENDER THIS INSTEAD 
                ...
                echo '<div class = "container mt-4" >You must <a href = "#logintopost" id = "login_prompt" >Login</a> first ,  to post a Comment in this Thread ! </div>';
    ...
    // GET THE COMMENTS POSTED IN THIS THREAD FROM DATABSE
        ...
        if($num == 0){
            echo '<div class="container text-center" style="margin : 30vh auto;">
            <h2>Oops ! No Comments found ! Why don\'t you be the one to <a href = "#logintopost"  id = "postComment_prompt" style = "text-decoration : none;"><i><strong >Post</strong></i></a> the First Comment?</h2>
            </div>';
        ...
        while($num-- > 0 && $row = mysqli_fetch_assoc($res)){
            echo '<div id="'.$row['comment_id'].'" class="comment container my-3 py-2 " style =  "word-wrap : break-word;">
                    ...
                    <div class="content col-md-10 col-sm-8">
                        <div class="fw-bold">By <a href = "./users.php?uid='.$row['owner_id'].'" style="text-decoration : underline;">'
...
<script>
    ...
    login_prompt.addEventListener('click', () => {
        logintopost.classList.toggle('invisible');
    });
